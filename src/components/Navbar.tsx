...
import { PackageOpen, Gavel, BarChart3, ShieldCheck, FileSpreadsheet, ArrowUpRight } from "lucide-react";
import { useCommerce } from "@/context/CommerceContext";

export function Navbar() {
  const pathname = usePathname();
  const { rfqs } = useCommerce();
  ...
  const navLinks = [
    ...
    { name: "Volume Pricing Matrix", href: "/pricing/", icon: BarChart3 },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1.5 sm:gap-3">
          ...
          <a
            ...
            className="px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 hover:text-white text-xs font-bold flex items-center gap-1.5 transition-all"
          >
            <span>Apex Hub</span>
